<?php
/**
 * Plugin Name: Magic Password Hashing
 * Version:     0.0.1
 * Description: Swaps out WordPress's password hashing mechanism with PHP 5.5's `password_hash()` functions set, and automatically rehashes the existing passwords on users next successful login.
 * Licence:     GPLv2 or later
 * Author:      Jascha Ehrenreich
 * Author URI:  https://jaeh.at
 */
...
if (!function_exists('wp_hash_password') && function_exists('password_hash')) :
   ...
   *  back to core password hashing mechanism if the password hash is of unknown
   *  format.
   *
...
function wp_password_hash_warn_incompatibility() {
  wp_password_hash_set_message('Your current system configuration does not support password hashing with password_hash() function. Please upgrade your PHP version to PHP 5.5 or later, or disable the "PHP native password hash" plugin.');
}
function wp_password_hash_warn_conflict() {
  wp_password_hash_set_message('Another plugin has already overridden the password hashing mechanism. The "Magic Password Hash" plugin will not work.');
}
